...
void LCD_write_data_8bit(u8 lcd_copy_word_8bit){
	...
	MGPIO_VidSetPinValue(lcd_ctrl_port, lcd_RS, HIGH);
	MGPIO_VidSetPinValue(lcd_ctrl_port, lcd_RW, LOW);
	// delay 1uS
	for(u8 i= 0; i <= 5; i++);
	// Enable pin is high to send the packet
	MGPIO_VidSetPinValue(lcd_ctrl_port, lcd_EN, HIGH);
	// delay 1uS
	for(u8 i= 0; i <= 5; i++);
	...
	for(u8 i= 0; i <= 7; i++){
		...
		MGPIO_VidSetPinValue(lcd_data_port, LCD_DataPins[i], BitValue);
	}
	// delay 1uS
	for(u8 i= 0; i <= 5; i++);
	// Enable pin low
	MGPIO_VidSetPinValue(lcd_ctrl_port, lcd_EN, LOW);
}
...
void LCD_write_cmd_8bit(u8 lcd_copy_cmd_8bit){
	...
	MGPIO_VidSetPinValue(lcd_ctrl_port, lcd_RS, LOW);
	MGPIO_VidSetPinValue(lcd_ctrl_port, lcd_RW, LOW);
	// delay 1uS
	for(u8 i= 0; i <= 5; i++);
	// Enable pin is high to send the packet
...
void LCD_function_set_4bit(void ){
	...
	for(u8 i= 0; i< 4; i++){
		...
		MGPIO_VidSetPinValue(lcd_data_port, LCD_DataPins[i+4], BitValue);
	}
	// Enable pin Low
	MGPIO_VidSetPinValue(lcd_ctrl_port, lcd_EN, LOW);
	// delay 50uS
	...
	for(u8 i= 0; i< 4; i++){
		...
		MGPIO_VidSetPinValue(lcd_data_port, i+4, BitValue);
	}
	// delay 1uS
	for(u8 i= 0; i <= 1; i++);
	// Enable pin low
...
void LCD_init(void){
	...
	for(u8 i= 0; i < 8; i++)
	{
		MGPIO_VidSetPinDirection(lcd_data_port, LCD_DataPins[i], OUTPUT_SPEED_10MHZ_PP);
	}
	// Configure the I/O control pins
	MGPIO_VidSetPinDirection(lcd_ctrl_port, lcd_EN, OUTPUT_SPEED_10MHZ_PP);
	MGPIO_VidSetPinDirection(lcd_ctrl_port, lcd_RW, OUTPUT_SPEED_10MHZ_PP);
	MGPIO_VidSetPinDirection(lcd_ctrl_port, lcd_RS, OUTPUT_SPEED_10MHZ_PP);
...
void LCD_set_cursor(u8 copy_u8_row, u8 copy_u8_column){
	 ...
	 */
#if LCD_interface == _8bit_interface
	...
	// wait more than 39 us
	 MSTK_VidBusyWaitUs(50);

#elif LCD_interface == _4bit_interface
	// calculate the addres from the given rows and columns
	u8 address= 128+(copy_u8_row*line2)+(copy_u8_column);
	// Sent set address command
